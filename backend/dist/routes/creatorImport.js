...
exports.creatorImportRouter = void 0;
const express_1 = require("express");
const requireAuth_1 = require("../middleware/requireAuth");
const Product_1 = require("../models/Product");
const firecrawl_1 = require("../lib/firecrawl");
const ImageAsset_1 = require("../models/ImageAsset");
const mongoose_1 = __importDefault(require("mongoose"));
const downloadImage_1 = require("../lib/downloadImage");
const cloudinary_1 = require("../lib/cloudinary");
exports.creatorImportRouter = (0, express_1.Router)();
exports.creatorImportRouter.use(requireAuth_1.requireAuth);
...
async function ownProductOrSend(req, res) {
...
exports.creatorImportRouter.post("/:productId/firecrawl/search", async (req, res) => {
    try {
        ...
        const results = await (0, firecrawl_1.firecrawlSearch)({ query, limit });
...
exports.creatorImportRouter.get("/:productId/assets", async (req, res) => {
    try {
        const product = await ownProductOrSend(req, res);
        if (!product)
            return;
        const assets = await ImageAsset_1.ImageAsset.find({ productId: product._id })
            .sort({ orderIndex: 1 })
            .lean();
...
exports.creatorImportRouter.post("/:productId/firecrawl/scrape", async (req, res) => {
    try {
        const product = await ownProductOrSend(req, res);
        if (!product)
            return;
...
exports.creatorImportRouter.post("/:productId/assets/ingest", async (req, res) => {
    try {
        const product = await ownProductOrSend(req, res);
        if (!product)
            return;
        ...
        const productId = new mongoose_1.default.Types.ObjectId(product._id);
        const creatorId = new mongoose_1.default.Types.ObjectId(req.user.id);
        let orderIndex = await ImageAsset_1.ImageAsset.countDocuments({ productId });
        ...
        for (const item of selected) {
            ...
            if (!url || typeof url !== "string")
                continue;
            try {
                const inspected = await (0, downloadImage_1.downloadAndInspectImage)(url);
                const uploadedToCloud = await (0, cloudinary_1.uploadImageBufferToCloudinary)({
                    buffer: inspected?.buffer,
                    folder,
                    fileNameNoExt: inspected?.fileNameNoExt,
                });
                const doc = await ImageAsset_1.ImageAsset.create({
                    productId,
                    creatorId,
                    source: {
                        sourceUrl: url,
        ...
        const newCount = await ImageAsset_1.ImageAsset.countDocuments({ productId });
