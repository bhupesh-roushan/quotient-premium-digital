...
bulkImportRouter.post("/json", async (req: AuthedRequest, res) => {
  try {
...
bulkImportRouter.post("/csv", async (req: AuthedRequest, res) => {
  try {
...
bulkImportRouter.post("/validate", async (req: AuthedRequest, res) => {
  try {
    const { products } = req.body;
    ...
    const validationResults = products.map((product, index) => {
      ...
      const duplicateSlug = products.find(
        (p, i) => i !== index && p.slug === product.slug && p.slug
      );
      if (duplicateSlug) {
...
// Get import template
bulkImportRouter.get("/template", async (req: AuthedRequest, res) => {
  try {
