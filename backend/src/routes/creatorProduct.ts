...
async function ensureUniqueSlug(baseSlug: string, checkExists: (slug: string) => Promise<boolean>): Promise<string> {
  let slug = baseSlug;
  let counter = 1;
...
creatorProductRouter.get("/stats", async (req: AuthedRequest, res) => {
  try {
    const creatorId = req.user!.id;
    console.log("Fetching stats for creator:", creatorId);

    const products = await Product.find({ creatorId }).lean();
...
creatorProductRouter.get("/revenue", async (req: AuthedRequest, res) => {
  try {
    const creatorId = req.user!.id;
    ...
    const revenueData = await Order.aggregate([
      ...
      {
        $match: {
          "product.creatorId": new mongoose.Types.ObjectId(creatorId),
        },
      },
      {
        $group: {
  ...
  try {
    ...
    if (req.body.codeFiles !== undefined) {
      if (!product.codeTemplate) {
        product.codeTemplate = {} as any;
      }
      (product.codeTemplate as any).codeFiles = req.body.codeFiles;
      if (req.body.framework) (product.codeTemplate as any).framework = req.body.framework;
      if (req.body.language) (product.codeTemplate as any).language = req.body.language;
      if (req.body.componentType) (product.codeTemplate as any).componentType = req.body.componentType;
      if (req.body.dependencies) (product.codeTemplate as any).dependencies = req.body.dependencies;
      if (req.body.hasLivePreview !== undefined) (product.codeTemplate as any).hasLivePreview = req.body.hasLivePreview;
      if (req.body.sandboxEnabled !== undefined) (product.codeTemplate as any).sandboxEnabled = req.body.sandboxEnabled;
    ...
    if (template && typeof template === "object") {
      ...
      if (template.compatibility !== undefined) templateUpdate.compatibility = template.compatibility;
      if (template.features !== undefined) templateUpdate.features = template.features;
      if (template.customizationLevel !== undefined) templateUpdate.customizationLevel = template.customizationLevel;
      if (template.includesAssets !== undefined) templateUpdate.includesAssets = template.includesAssets;
      
      Object.assign((product as any).template, templateUpdate);
    ...
    if (req.body.componentDeliverables !== undefined && Array.isArray(req.body.componentDeliverables)) {
      ...
      (product as any).template.componentDeliverables = req.body.componentDeliverables;
      product.markModified('template');
    }
...
// Delete Product
creatorProductRouter.delete("/:productId", async (req: AuthedRequest, res) => {
  try {
  ...
  try {
    ...
    const transformedProducts = products.map(product => {
      ...
      const previewImages = (product.deliverables || [])
        .filter((d: any) => d.url && d.url.includes('http') && (d.url.includes('.jpg') || d.url.includes('.png') || d.url.includes('.jpeg') || d.url.includes('.webp')))
        .slice(0, 3)
        .map((d: any) => ({
          url: d.url,
          _id: (d as any)._id ? (d as any)._id.toString() : new mongoose.Types.ObjectId().toString()
        }));

      // Add cover image if available
      const coverImageAsset = product.coverImageAssetId as any;
      if (coverImageAsset && coverImageAsset.cloudinary && coverImageAsset.cloudinary.secureUrl) {
        previewImages.unshift({
          ...
          _id: coverImageAsset._id ? coverImageAsset._id.toString() : new mongoose.Types.ObjectId().toString()
        });
      }
...
creatorProductRouter.get("/:productId/assets", async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
...
import { uploadFileBufferToCloudinary } from "../lib/cloudinary";
import { v2 as cloudinary } from "cloudinary";

const upload = multer({ storage: multer.memoryStorage() });

creatorProductRouter.post("/:productId/upload-file", upload.single("file"), async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
...
creatorProductRouter.post("/assets", async (req: AuthedRequest, res) => {
  try {
    ...
    const savedAsset = await ImageAsset.create({
      productId: product._id,
      creatorId: req.user!.id,
      source: {
        sourceUrl,
...
creatorProductRouter.post("/:productId/upload-image", async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
    ...
    // Create a new asset
    const newAsset = await ImageAsset.create({
      productId: product._id,
      creatorId: req.user!.id,
      source: {
        sourceUrl: imageUrl,
...
creatorProductRouter.post("/:productId/upload-image-file", upload.single("file"), async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
...
// Set Cover Image
creatorProductRouter.patch("/:productId/cover", async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
...
// Delete Asset
creatorProductRouter.delete("/:productId/assets/:assetId", async (req: AuthedRequest, res) => {
  try {
    ...
    if (imageAsset.cloudinary?.publicId) {
      try {
        await cloudinary.uploader.destroy(imageAsset.cloudinary.publicId);
    ...
    await ImageAsset.findByIdAndDelete(assetId);
...
// Publish product
creatorProductRouter.patch("/:productId/publish", async (req: AuthedRequest, res) => {
  try {
    const { productId } = req.params;
